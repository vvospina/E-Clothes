...
def consejos_list_create(request):
    ...
    if request.method == "GET":
        # Captura el parámetro de consulta ?categoria= de la URL
        # (Ej: /api/v1/consejos?categoria=Reutilización)
        categoria = request.GET.get("categoria")

        # Construye el filtro para MongoDB: si mandaron categoría
        # busca por ella, si no, trae todo ({})
        ...
        # Transforma el formato del ID interno de MongoDB (_id)
        # a un string estándar ("id") para el JSON
        for c in consejos:
            c["id"] = str(c.pop("_id"))
...
def consejo_detail(request, pk):
    ...
    """
    try:
        # Convierte el parámetro 'pk' de la URL en un ObjectId válido
        # de MongoDB y realiza la búsqueda
        ...
        # Si el ID proporcionado en la URL no tiene el formato correcto
        # que MongoDB espera
...
def health(request):
    """
    Endpoint público para verificar el estado del microservicio
    (usado en Docker / Compose).
    """
